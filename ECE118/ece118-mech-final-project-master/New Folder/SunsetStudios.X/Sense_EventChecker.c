...
uint8_t CheckFLBumper(void) {
    ...
    if ((curEvent != lastEvent) && (AD_ReadADPin(BAT_VOLTAGE) > BATTERY_DISCONNECT_THRESHOLD)) {
        thisEvent.EventType = curEvent;
...
#define TAPE_DARK_THRESHOLD_C 800 // BVK increased from 800 to try and stop sensor 0x04 from raeding tape when should be white
#define TAPE_DARK_THRESHOLD_D 800
#define TAPE_DARK_THRESHOLD_E 700
#define TAPE_DARK_THRESHOLD_F 800

#define TAPE_LIGHT_THRESHOLD_A 300
#define TAPE_LIGHT_THRESHOLD_B 300
#define TAPE_LIGHT_THRESHOLD_C 300
#define TAPE_LIGHT_THRESHOLD_D 300
#define TAPE_LIGHT_THRESHOLD_E 300
#define TAPE_LIGHT_THRESHOLD_F 300
...
//Combined Check for Dark and and Light
//Posts events whenever there is a change in Light state
static uint16_t lastDarkState = DEFAULT_STATE;

uint8_t CheckBottomTapeforDark(void) {
    //Default State is DARK_DETECTED unpressed
    static ES_EventTyp_t lastEvent = DF_NONE_DETECTED; // ?? BVK - Dark floor - none detected? means... we see only white? or we see only black?
    ...
    uint16_t currentLightState = lastDarkState;
    ...
    if (tapeReading > TAPE_DARK_THRESHOLD_A) {
        ...
        curEvent = FLOOR_LIGHT_CHANGE;
        currentLightState = currentLightState | A_TS_MASK;
    } else if ((tapeReading < TAPE_LIGHT_THRESHOLD_A)) {
        curEvent = FLOOR_LIGHT_CHANGE;
        currentLightState = currentLightState & NOT_TS_MASK_A;
    }
    ...
    } else if ((tapeReading < TAPE_LIGHT_THRESHOLD_B)) {
        curEvent = FLOOR_LIGHT_CHANGE;
        currentLightState = currentLightState & NOT_TS_MASK_B;
    }
    ...
    } else if ((tapeReading < TAPE_LIGHT_THRESHOLD_C)) {
        curEvent = FLOOR_LIGHT_CHANGE;
        currentLightState = currentLightState & NOT_TS_MASK_C;
    }
    ...
    } else if ((tapeReading < TAPE_LIGHT_THRESHOLD_D)) {
        curEvent = FLOOR_LIGHT_CHANGE;
        currentLightState = currentLightState & NOT_TS_MASK_D;
    }
    //Set Current Event to UNPRESSED when Bump State is LOW
    ...
    } else if ((tapeReading < TAPE_LIGHT_THRESHOLD_E)) {
        curEvent = FLOOR_LIGHT_CHANGE;
        currentLightState = currentLightState & NOT_TS_MASK_E;
    }
    ...
    } else if ((tapeReading < TAPE_LIGHT_THRESHOLD_F)) {
        curEvent = FLOOR_LIGHT_CHANGE;
        currentLightState = currentLightState & NOT_TS_MASK_F;
    }
    // BVK EDITED TO ADD PARANTHESE AROUND OR STATEMENT 5/31 3PM
    if (((curEvent != lastEvent) || (currentLightState != lastDarkState)) && (AD_ReadADPin(BAT_VOLTAGE) > BATTERY_DISCONNECT_THRESHOLD)) {
        thisEvent.EventType = curEvent;
...
uint8_t CheckBottomTapeforLight(void) {
    ...
    //Default State is DARK_DETECTED 
 ...
 * @author Amber */
#define TAPE_DARK_THRESHOLD_FRONT 100
#define TAPE_LIGHT_THRESHOLD_FRONT 40

#define TAPE_DARK_THRESHOLD_MID 250
#define TAPE_LIGHT_THRESHOLD_MID 60

#define TAPE_DARK_THRESHOLD_REAR 100
#define TAPE_LIGHT_THRESHOLD_REAR 40

uint8_t CheckTowerTape(void) {
    //Default State for Tower Tape is LW_DETECTED

    static ES_EventTyp_t lastEvent = LW_DETECTED;
    static char lastLightState = 0;
    ES_EventTyp_t curEvent;
    ES_Event thisEvent;

    //Initialize retun value as FALSE
    uint8_t returnVal = FALSE;
    unsigned int tapeReading = AD_ReadADPin(TS_FRONT_AD);

    char currentLightState = 0;
    if (tapeReading > TAPE_DARK_THRESHOLD_FRONT) {
        //                    printf("\r\nEntered Here in event A Statement");
        curEvent = DW_DETECTED;
        currentLightState = currentLightState | TOWER_TAPE_FRONT;
    } else if ((tapeReading < TAPE_LIGHT_THRESHOLD_FRONT)) {
        curEvent = DW_DETECTED;
        currentLightState = currentLightState & DDW;
    }
    tapeReading = AD_ReadADPin(TS_MID_AD);
    if (tapeReading > TAPE_DARK_THRESHOLD_MID) {
        //                    printf("\r\nEntered Here in event A Statement");
        curEvent = DW_DETECTED;
        currentLightState = currentLightState | TOWER_TAPE_MID;
    } else if ((tapeReading < TAPE_LIGHT_THRESHOLD_MID)) {
        curEvent = DW_DETECTED;
        currentLightState = currentLightState & TAPE_ON_WALL;
    }
    tapeReading = AD_ReadADPin(TS_REAR_AD);
    if (tapeReading > TAPE_DARK_THRESHOLD_REAR) {
        //                    printf("\r\nEntered Here in event A Statement");
        curEvent = DW_DETECTED;
        currentLightState = currentLightState | TOWER_TAPE_REAR;
    } else if ((tapeReading < TAPE_LIGHT_THRESHOLD_REAR)) {
        curEvent = DW_DETECTED;
        currentLightState = currentLightState & WDD;
    }
    ...
    if ((curEvent != lastEvent || (currentLightState != lastLightState)) && (AD_ReadADPin(BAT_VOLTAGE) > BATTERY_DISCONNECT_THRESHOLD)) {
        // bvk 6/2 -- spamming events, goinng to update history if it changes state
        lastLightState = currentLightState;
        thisEvent.EventType = curEvent;
        thisEvent.EventParam = currentLightState;
        returnVal = TRUE;
...
//Created 6/1/22 Amber (Combining Event Checker For Track Wire A and B)

uint8_t CheckBothTrackWire(void) {
    //Default State for TrackWire is 
    static ES_EventTyp_t lastEvent = TW_NOT_DETECTED;
    static char lastTrackWireState = 0;
    ES_EventTyp_t curEvent;
    ES_Event thisEvent;

    //Initialize retun value as FALSE
    uint8_t returnVal = FALSE;
    char curTrackWireState;
    curTrackWireState = Read_All_TrackWire();
    //If greater than 1 Set to TW detected
    if (curTrackWireState) {
        curEvent = TW_DETECTED;
    }//When both are low, set to TW not Detected
    else {
        curEvent = TW_NOT_DETECTED;
    }
    /*If there is a state transition, set thisEvent to curEvent
     **/
    //Post Event When new event or when change in track wire state
    if ((curEvent != lastEvent || (curTrackWireState != lastTrackWireState)) && (AD_ReadADPin(BAT_VOLTAGE) > BATTERY_DISCONNECT_THRESHOLD)) {
        thisEvent.EventType = curEvent;
        thisEvent.EventParam = curTrackWireState;
        returnVal = TRUE;
        lastEvent = curEvent;
        lastTrackWireState = curTrackWireState;
        PostTopHSM(thisEvent);
    }
...
uint8_t CheckforBeaconDigital(void) {
    //Default State for TrackWire is // this is beacon detector
    static ES_EventTyp_t lastEvent = BEACON_NOT_WITHIN_DIGITAL_RANGE;
    ...
    // BVK EDITED SO WE DONT HAVE A BOOT UP EVENT WHEN WE POWER ON
    if (curEvent != lastEvent && (AD_ReadADPin(BAT_VOLTAGE) > BATTERY_DISCONNECT_THRESHOLD)) { // COULD SIMPLY HAVE A BOOT UP BOOLEAN 
        thisEvent.EventType = curEvent;
        thisEvent.EventParam = TRUE; // 1.. ? ok
        returnVal = TRUE;
        lastEvent = curEvent;
        //        SaveEvent(thisEvent);
        //Uncomment and change Name of Service function once Service file is created 
...
static enum beacon_state {
    NO_BEAC, YES_BEAC
} last_beacon_state = NO_BEAC;
 ...
 * @author Amber */
uint8_t CheckforBeaconAnalog(void) { //Default State is BEACON_NOT_WTITHIN_ANALOG_RANGE
    static ES_EventTyp_t lastEvent = BEACON_NOT_WITHIN_ANALOG_RANGE;

    //Defining this event 
    ES_EventTyp_t curEvent = lastEvent;
    ES_Event thisEvent;
    //    enum beacon_state cur_beac_state = NO_BEAC;    
    //Initialize retun value as FALSE
    uint8_t returnVal = FALSE;
    static unsigned int beaconReading = 0;

    //Get current value of the Beacon Detector // BVK- need 
    if (AD_IsNewDataReady()) {
        beaconReading = Read_Analog_Beacon();
    }
    //    unsigned int beaconReading = Read_Analog_Beacon();

    if (beaconReading < BEACON_RANGE_LOW) {
        //        cur_beac_state = NO_BEAC;
        curEvent = BEACON_NOT_WITHIN_ANALOG_RANGE;
    }// hopefully more secure
    else if (beaconReading > BEACON_RANGE_HIGH) {
    ...
    //        curEvent = BEACON_WITHIN_ANALOG_RANGE;
    //        LED_OnBank(LED_BANK2, 1);
    //    }//Set Current Event to UNPRESSED when Bump State is LOW
    //    else if (beaconReading < BEACON_RANGE_LOW) {
    //        curEvent = BEACON_NOT_WITHIN_ANALOG_RANGE;
    //        LED_OffBank(LED_BANK2, 1);
    //    }
    //    else{
    //        curEvent = 
    //    }
    //    if(last_beacon_state != cur_beac_state){
    //        if(cur_beac_state){ // enum 1
    //            curEvent = BEACON_WITHIN_ANALOG_RANGE;
    //        }
    //        else{
    //            curEvent = BEACON_NOT_WITHIN_ANALOG_RANGE;
    //        }
    //        
    //        last_beacon_state = cur_beac_state;
    //    }
 ...
 * Will return TRUE if the specified Reading appears in the EventParam
 * Suggested to be used with NOT_TS_MASKS and TS_MASKS
 * @author Amber */
#define ADE_NOT_MASK (A_TS_MASK | D_TS_MASK | E_TS_MASK )
#define ADE_MASK (NOT_TS_MASK_A & NOT_TS_MASK_D &NOT_TS_MASK_E)

char CompareToEventParam(uint8_t Reading, uint8_t EventParam) {
    /*For bits that are 1,
     *  AND EventParam with ADE mask, set to ResultParam
     *  AND ResultParam with Reading set to ResultParam
     *  XOR ResutParam with Reading
     * For bits that are 0 
     *  OR EventParam with ALL_TAPE_HIGH
     */
    uint8_t match_1 = ((EventParam & ADE_NOT_MASK) ^ (Reading & ADE_NOT_MASK));
    uint8_t match_0 = ((EventParam | ADE_MASK) ^ (Reading | ADE_MASK));
