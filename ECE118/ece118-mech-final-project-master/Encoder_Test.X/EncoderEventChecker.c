 ...
 * PUBLIC FUNCTIONS                                                            *
 ******************************************************************************/
static uint8_t prev_hall_right_a;
static uint8_t prev_hall_right_b;
static uint8_t prev_hall_left_a;
static uint8_t prev_hall_left_b;
/**
...
uint8_t CheckEncoderPWM(void) {
    ...
    ES_Event thisEvent;// = ES_NO_EVENT;
    ...
    uint8_t cur_hall_right_a = HALL_IN_RA; // read the pwm input pin for right side
    uint8_t cur_hall_right_b = HALL_IN_RB; // read the pwm input pin for right side
    uint8_t cur_hall_left_a = HALL_IN_LA; // read the pwm in-put pin for the left side
    uint8_t cur_hall_left_b = HALL_IN_LB; // read the pwm in-put pin for the left side        
...
//    printf("\r\ncurrent hall left = %d", cur_hall_left);
    ...
    if ((prev_hall_right_a != cur_hall_right_a) || (prev_hall_right_b != cur_hall_right_b)) {
//    if(cur_hall_left_a == cur_hall_left_b) { 
...
//        printf("posted right");
    ...
    prev_hall_right_a = cur_hall_right_a; // update history
    prev_hall_right_b = cur_hall_right_b; // update history
    
    // Left in the same checker?
    // check the left PWM for a rising edge
    if ((prev_hall_left_a != cur_hall_left_a) || (prev_hall_left_b != cur_hall_left_b)) {
//    if(cur_hall_left_a == cur_hall_left_b) {   
        curEvent = ENCODER_TICK;
...
//        printf("posted left");
    ...
    prev_hall_left_a = cur_hall_left_a; // update history
    prev_hall_left_b = cur_hall_left_b; // update history
