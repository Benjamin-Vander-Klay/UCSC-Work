...
#define WAIT_BTN_TESTS 200 // ms break btn tests to limit kickback? 
#define WAIT_TESTS 1500 // 1.5k milli seconds is the diraction of each motor test
#define FORWARD 1
#define REVERSE 0

#define GRADUAL_TURN_CONST 525 // difference btn wheel speeds for Gradual Turn
...
char Drive_Init() {
    ...
    // encoders 
    if (IO_PortsSetPortInputs(HALL_IN_PORT, HALL_IN_PIN_RA | HALL_IN_PIN_RB | HALL_IN_PIN_LA | HALL_IN_PIN_LA) == ERROR) {// Pin X3 on I/O shield
...
char Drive_Straight(int speed) {
    ...
    int polarity ;
    // if negative, set polarity negative, if posititve, set positive.
    if(speed < 0){
        polarity = -1;
    }
    else{
    ...
    // minimum pwm where the motor still turns
    // between -300 and 300 will cap to either -300 or 300 so that we still turn
    if (abs(speed) < MIN_SPEED) {
        abs_speed = MIN_SPEED;
    }
    speed = abs_speed * polarity;
...
char Drive_Stop(uint8_t which_motor) {
    ...
    if(which_motor == RIGHT | BOTH | BOTH_NO_RESET){
        // right motor 
//        if (PWM_SetDutyCycle(PWM_PORTZ06, MIN_PWM) == ERROR) {// Pin Z6 on I/O shield
//            printf("ERROR in Drive_Right_Motor calling PWM_SetDutyCycle(PWM_PORTZ06, pwm_input)\n");
//            return ERROR;
//        }
        if(Drive_Right_Motor(MIN_PWM) == ERROR){
            printf("\r\nERROR in Drive_Stop Left motor");
        }
        
    }
    if(which_motor == LEFT | BOTH | BOTH_NO_RESET){
        // left motor
...
//        } 
        // minimum pwm where the motor still turns
        if(Drive_Left_Motor(MIN_PWM) == ERROR){
            printf("\r\nERROR in Drive_Stop Left motor");
        }
    }
    
    if(which_motor == BOTH){
        //reset the encoder ticks .. this requires access to EncoderService and Configure.h
        // limits the modularity of the library but makes it a little easier for the state machine
        // :)
       ES_Event DriveEvent;
       DriveEvent.EventType = ENCODER_RESET; // rotates 180 degrees to search for tower in that specific quadrant ** will need 
...
char Gradual_Turn_Right(int speed) {
    ...
    // minimum pwm where the motor still turns after subtraction. 
    if (abs_speed < GRADUAL_TURN_MIN) {
        abs_speed = GRADUAL_TURN_MIN;
    }
    // maintain the polarity while still allowing us to create the gradual turn speeds
    int abs_speed_grad = abs_speed - GRADUAL_TURN_CONST;
    int speed_diff = abs_speed_grad * polarity;
    
    speed = abs_speed * polarity;
    
    if (Drive_Left_Motor(speed_diff) == ERROR) {
        error = ERROR;
    }
    if (Drive_Right_Motor(speed) == ERROR) {
        error = ERROR;
...
char Hard_Turn_Right(int speed) {
    ...
    // maintain the polarity while still allowing us to create the gradual turn speeds
    int abs_speed_hard = abs_speed - HARD_TURN_CONST;
    int speed_diff = abs_speed_hard * polarity;
...
char Hard_Turn_Left(int speed) {
    ...
    if (Drive_Right_Motor(speed_diff) == ERROR) {
        error = ERROR;
    }
    return error;
}
...
#ifdef MOTOR_TEST
// REQUIRES TIMERS.H!!!!
// COULD SWAP TO LIKE 50K NOPS THO
int main(void) {
    // Taken from my lab3 
...
//    Drive_Tank_Turn_Right(-PWM_SPEED);
    ...
    printf("\r\nDrive_Straight");
    Drive_Straight(DRIVE_MAX);
    wait = TIMERS_GetTime() + WAIT_TESTS;
     printf("\r\nDuty Cycle LEFT = %d\n", PWM_GetDutyCycle(EN_B));
    while(TIMERS_GetTime() < wait);
    
    printf("\r\nDrive_Straight Slower");
    Drive_Straight(DRIVE_MED);
    wait = TIMERS_GetTime() + WAIT_TESTS;
     printf("\r\nDuty Cycle LEFT = %d\n", PWM_GetDutyCycle(EN_B));
    while(TIMERS_GetTime() < wait);
//    Drive_Stop(BOTH);
    ...
    printf("\r\nDrive_STOP");
    Drive_Stop(BOTH);
    wait = TIMERS_GetTime() + WAIT_BTN_TESTS;
    while(TIMERS_GetTime() < wait);
    
    printf("\r\nDrive_Straight REVERSE");
    Drive_Straight(-PWM_SPEED);
    wait = TIMERS_GetTime() + WAIT_TESTS;
     printf("\r\nDuty Cycle LEFT = %d\n", PWM_GetDutyCycle(EN_B));
    while(TIMERS_GetTime() < wait);
    
    printf("\r\nDrive_Straight REVERSE Slower");
    Drive_Straight(-DRIVE_MED);
    wait = TIMERS_GetTime() + WAIT_TESTS;
    printf("\r\nDuty Cycle LEFT (unison) = %d\n", PWM_GetDutyCycle(EN_B));
    while(TIMERS_GetTime() < wait);
    ...
    printf("\r\n\r\n **** \r\n Blocking Drive Test Harness COMPLETE!! :)\r\n ****");
