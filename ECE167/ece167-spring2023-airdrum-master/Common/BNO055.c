...
#define ACC_CONFIG_PARAMS (0b00011110) //+/-2g, 62.5 Hz BW // 0x18 was defaultBVK changing to be 4 for testing. see page 27 of datasheet // 0x1A gives 62.5 and 8 g's
...
char BNO055_Init(unsigned char addr) {
    ...
    byteReturn = I2C_ReadRegister(addr, BNO055_CHIP_ID_ADDR);
    ...
    byteReturn = I2C_WriteReg(addr, BNO055_OPR_MODE_ADDR, OPERATION_MODE_CONFIG);
    DelayMicros(250000); // delay between changing op modes > 19 msec
    /*set the register page to page 1*/
    byteReturn = I2C_WriteReg(addr, BNO055_PAGE_ID_ADDR, BNO055_PAGE1);
    /* Config gyro for 250 dps */
    byteReturn = I2C_WriteReg(addr, BNO055_GYR_CONFIG_0, GYRO_CONFIG_PARAMS_0);
    /* Config accelerometer to +/- 2g */
    byteReturn = I2C_WriteReg(addr, BNO055_ACC_CONFIG, ACC_CONFIG_PARAMS);
    /*set the register page to page 0*/
    byteReturn = I2C_WriteReg(addr, BNO055_PAGE_ID_ADDR, BNO055_PAGE0);
    /* remap axes */
//    unsigned char prev_map = I2C_ReadRegister(addr, BNO055_AXIS_MAP_CONFIG_ADDR);
//    unsigned char new_map = (prev_map & 0b11000000) | 0b00100001;
//    byteReturn = I2C_WriteReg(addr, BNO055_AXIS_MAP_CONFIG_ADDR, new_map);
//    byteReturn = I2C_WriteReg(addr, BNO055_AXIS_MAP_SIGN_ADDR, 0x00);
    /*set units*/
    byteReturn = I2C_WriteReg(addr, BNO055_UNIT_SEL_ADDR, UNITS_PARAM);
    /* Set operation mode to IMU */
    byteReturn = I2C_WriteReg(addr, BNO055_OPR_MODE_ADDR, OPERATION_MODE_IMUPLUS); // was OPERATION_MODE_AMG
    return (byteReturn);
}

unsigned char BNO055_ReadCalibStat(unsigned char addr) {
    return(I2C_ReadRegister(addr, BNO055_CALIB_STAT_ADDR));
}

/**
 * @Function BNO055_Read*()
 * @param None
 * @return Returns processed fusion sensor reading
 * @brief reads the calculated fusion data as given by name,
 * requires OPERATION_MODE_IMUPLUS or other fusion mode.
 * @author Anthony A. Russo*/
int BNO055_ReadEulHeading(unsigned char addr) {
    return(I2C_ReadInt(addr, BNO055_EULER_H_LSB_ADDR, 0));
}
...
int BNO055_ReadEulPitch(unsigned char addr) {
    return(I2C_ReadInt(addr, BNO055_EULER_P_LSB_ADDR, 0));
}
...
int BNO055_ReadEulRoll(unsigned char addr) {
    return(I2C_ReadInt(addr, BNO055_EULER_R_LSB_ADDR, 0));
}
...
int BNO055_ReadAccelX(unsigned char addr) {
    return (I2C_ReadInt(addr, BNO055_ACCEL_DATA_X_LSB_ADDR, 0));
}
...
int BNO055_ReadAccelY(unsigned char addr) {
    return (I2C_ReadInt(addr, BNO055_ACCEL_DATA_Y_LSB_ADDR, 0));
}
...
int BNO055_ReadAccelZ(unsigned char addr) {
    return (I2C_ReadInt(addr, BNO055_ACCEL_DATA_Z_LSB_ADDR, 0));
}
...
int BNO055_ReadGyroX(unsigned char addr) {
    return (I2C_ReadInt(addr, BNO055_GYRO_DATA_X_LSB_ADDR, 0));
}
...
int BNO055_ReadGyroY(unsigned char addr) {
    return (I2C_ReadInt(addr, BNO055_GYRO_DATA_Y_LSB_ADDR, 0));
}
...
int BNO055_ReadGyroZ(unsigned char addr) {
    return (I2C_ReadInt(addr, BNO055_GYRO_DATA_Z_LSB_ADDR, 0));
}
...
int BNO055_ReadMagX(unsigned char addr) {
    return (I2C_ReadInt(addr, BNO055_MAG_DATA_X_LSB_ADDR, 0));
}
...
int BNO055_ReadMagY(unsigned char addr) {
    return (I2C_ReadInt(addr, BNO055_MAG_DATA_Y_LSB_ADDR, 0));
}
...
int BNO055_ReadMagZ(unsigned char addr) {
    return (I2C_ReadInt(addr, BNO055_MAG_DATA_Z_LSB_ADDR, 0));
}
...
int BNO055_ReadTemp(unsigned char addr) {
    return (I2C_ReadRegister(addr, BNO055_TEMP_ADDR));
}
